 ...
 * Uses Claude (Anthropic SDK) instead of GPT-4o-mini.
...
const SYSTEM_PROMPT = `You are ShipKit's business analyst. Given a business idea, generate a structured analysis that serves as an interactive business brief.
...
Your response MUST be valid JSON with this exact structure:
{
  ...
  "message": "An enthusiastic 2-3 sentence summary of what you just created, ending with an invitation to get the full ShipKit"
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!responseText) {
      console.error('[ShipKit Analyze] Empty response from Claude');
      return NextResponse.json(
    ...
    try {
      ...
      console.error('[ShipKit Analyze] Failed to parse Claude response:', parseError);
      console.error('[ShipKit Analyze] Raw response:', responseText);
      return NextResponse.json(
    ...
    try {
      await prisma.chat_submissions.create({
        data: {
          ...
          recommended_product: 'shipkit',
          analysis_json: JSON.parse(JSON.stringify(analysis)),
        },
      });
      console.log(`[ShipKit Analyze] Saved analysis with sessionId: ${sessionId}`);
    } catch (dbError) {
      console.error('[ShipKit Analyze] Failed to save to DB:', dbError);
    ...
    console.error('[ShipKit Analyze] Error:', error);
    return NextResponse.json(
