...
export interface CodegenInput {
  ...
  brandIdentity: string; // Output from sk_brand_identity_03
  appArchitecture: string; // Output from sk_app_architecture_07
  codebaseSpec: string; // Output from sk_nextjs_codebase_08
}
...
async function generateScaffold(input: CodegenInput): Promise<Map<string, string>> {
  ...
  const response = await anthropic.messages.create({
    ...
    system: `You are a Next.js expert. Generate a complete, minimal Next.js application scaffold. Output each file as:
\`\`\`filepath: path/to/file
file contents here
\`\`\`

Use TypeScript, Tailwind CSS, and modern React patterns. Apply the brand identity provided.`,
    messages: [{
      role: 'user',
      content: `Generate a Next.js app scaffold for:

Business: ${input.businessConcept.substring(0, 500)}

Brand Identity Summary: ${input.brandIdentity.substring(0, 1000)}
...
async function pushToGitHub(
  ...
  const { data: repo } = await octokit.repos.createForAuthenticatedUser({
    name: repoName,
    description: `Generated by ShipKit — ${projectName}`,
    private: false,
  ...
  const { data: commit } = await octokit.git.createCommit({
    owner: repo.owner.login,
    repo: repoName,
    message: 'Initial codebase generated by ShipKit',
    tree: tree.sha,
...
/**
 * Push generated files to GitHub under the ShipKit organization.
