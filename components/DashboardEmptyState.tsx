...
export function DashboardEmptyState({ userName }: DashboardEmptyStateProps) {
  ...
  if (timedOut) {
    return (
      <div className="py-16 text-center max-w-md mx-auto">
        <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-accent/10 border border-border flex items-center justify-center">
          <svg className="w-10 h-10 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        </div>
        <h3 className="text-xl font-bold text-white mb-3">Still setting up...</h3>
        <p className="text-fsvc-text-secondary mb-2">
          Your project is still being created. This can take a minute.
        </p>
        <p className="text-sm text-fsvc-text-disabled mb-6">
          We&apos;re checking automatically. You can also refresh manually.
        </p>
        <button
          onClick={() => router.refresh()}
          className="px-6 py-2.5 rounded-lg bg-surface border border-border text-white text-sm font-medium hover:bg-raised transition-colors"
        >
          Refresh now
        </button>
      </div>
  ...
  return (
    <div className="py-16 text-center max-w-md mx-auto">
      <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-accent/10 border border-border flex items-center justify-center">
        <svg className="w-10 h-10 text-accent-2 animate-spin" fill="none" viewBox="0 0 24 24">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" fill="none" />
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
        </svg>
      </div>
      <h3 className="text-xl font-bold text-white mb-3">Setting up your project...</h3>
      <p className="text-fsvc-text-secondary mb-2">
